...
export class DiscordAlerter {
      ...
      try {
        ...
        if (response) {
          ...
          if (response.length > 200) {
            await interaction.editReply({ embeds: [this._embed('PhantomGraph', response, 0x00FF66)] });
          } else {
  ...
  _buildAlertEmbed(evaluation, delta, tier) {
    ...
    const embed = this._embed(
      `${tierEmoji} PHANTOMGRAPH ${tc.label}`,
      `**${evaluation.headline}**\n\n${evaluation.reason}`,
      tc.color
    );
    ...
    if (embed.setFields) {
      ...
      embed.setFooter({ text: `PhantomGraph Intelligence · ${new Date().toISOString().replace('T', ' ').substring(0, 19)} UTC` });
    } else {
      // Raw embed object for webhook fallback
      embed.fields = fields;
      embed.footer = { text: `PhantomGraph Intelligence · ${new Date().toISOString().replace('T', ' ').substring(0, 19)} UTC` };
    }
