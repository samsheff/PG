...
// USP feature: PhantomGraph becomes a conversational intelligence agent via Telegram
...
export class TelegramAlerter {
  ...
  _buildEvaluationPrompt() {
    return `You are PhantomGraph, an elite intelligence alert evaluator for a personal OSINT monitoring system. You analyze signal deltas from a 25-source intelligence sweep and decide if the user needs to be alerted via Telegram.
...
Respond with ONLY valid JSON:
{
  ...
  _formatTieredAlert(evaluation, delta, tier) {
    ...
    const lines = [
      `${tc.emoji} *PHANTOMGRAPH ${tc.label}*`,
      ``,
      `*${evaluation.headline}*`,
      ``,
      evaluation.reason,
